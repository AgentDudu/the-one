...
public class DTNSimGUI extends DTNSimUI {
  ...
  private void initGUI() {
    ...
    int trailLength = guiSettings.getInt("nodeTrailLength", 50);
    String trailColorStr = guiSettings.getSetting("nodeTrailColor", "128,128,128");
    boolean trailFade = guiSettings.getBoolean("nodeTrailFadeEnabled", true);

    Color defaultTrailColor = Color.GRAY;
    try {
      String[] rgb = trailColorStr.split(",");
      if (rgb.length == 3) {
        defaultTrailColor = new Color(Integer.parseInt(rgb[0].trim()),
            Integer.parseInt(rgb[1].trim()),
            Integer.parseInt(rgb[2].trim()));
      } else {
        System.err.println("Invalid GUI.nodeTrailColor format: " + trailColorStr + ". Using default gray.");
      }
    } catch (NumberFormatException | ArrayIndexOutOfBoundsException e) {
      System.err.println(
          "Error parsing GUI.nodeTrailColor: " + trailColorStr + ". Using default gray. Error: " + e.getMessage());
    }

    NodeGraphic.configureTrails(trailsEnabled, trailLength, defaultTrailColor, trailFade);

    this.field = new PlayField(world, this);

    this.field.updateFieldSize();
    ...
    this.main = new MainWindow(this.scen.getName(), world, this.field,
