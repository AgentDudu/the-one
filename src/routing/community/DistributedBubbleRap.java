...
public class DistributedBubbleRap
  ...
  /** Settings required for getArrayCentrality if recalculating */
  ...
  /** Default values matching CWindowCentrality for consistency */
  protected static int CENTRALITY_TIME_WINDOW = 86400; // 24 hours
  protected static int EPOCH_COUNT = 5; // Default for CWindowCentrality
  ...
  public DistributedBubbleRap(Settings s) {
    ...
    if (centralitySettings.contains(CENTRALITY_WINDOW_SETTING))
      CENTRALITY_TIME_WINDOW = centralitySettings.getInt(CENTRALITY_WINDOW_SETTING);
    if (centralitySettings.contains(EPOCH_COUNT_SETTING))
      EPOCH_COUNT = centralitySettings.getInt(EPOCH_COUNT_SETTING);
  }
  ...
  public DistributedBubbleRap(DistributedBubbleRap proto) {
    ...
    CENTRALITY_TIME_WINDOW = proto.CENTRALITY_TIME_WINDOW;
    EPOCH_COUNT = proto.EPOCH_COUNT;
  }
  ...
  /**
   * Calculates and returns an array representing the count of unique contacts
   * within successive time windows, ending at the current simulation time.
   * This implementation mirrors the counting logic of CWindowCentrality.
   * The size of the array is determined by EPOCH_COUNT.
   * array[0] = count in most recent window
   * array[1] = count in second most recent window, etc.
   * 
   * @return An int array of contact counts per window, or null if history is
   *         empty.
   */
  @Override
  public int[] getArrayCentrality() {
    if (connHistory == null || connHistory.isEmpty()) {
      return new int[EPOCH_COUNT];
    }

    int[] windowCounts = new int[EPOCH_COUNT];
    ...
    for (Map.Entry<DTNHost, List<Duration>> entry : connHistory.entrySet()) {
      ...
      for (int i = durations.size() - 1; i >= 0; i--) {
        ...
        int epochIndex = timePassed / CENTRALITY_TIME_WINDOW;

        if (epochIndex >= EPOCH_COUNT)
          continue;

        Set<DTNHost> countedInThisEpoch = nodesCountedInEpoch.get(epochIndex);
        if (!countedInThisEpoch.contains(peer)) {
          windowCounts[epochIndex]++;
          countedInThisEpoch.add(peer);
        }
      }
    }
    return windowCounts;
