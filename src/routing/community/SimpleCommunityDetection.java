...
public class SimpleCommunityDetection implements CommunityDetection {
  ...
  public SimpleCommunityDetection(Settings s) {
    this.lambda = s.contains(LAMBDA_SETTING) ? s.getDouble(LAMBDA_SETTING) : 0.5;
    this.gamma = s.contains(GAMMA_SETTING) ? s.getDouble(GAMMA_SETTING) : 0.5;
    this.familiarThreshold = s.contains(FAMILIAR_SETTING) ? s.getDouble(FAMILIAR_SETTING) : 3600.0;
  }
