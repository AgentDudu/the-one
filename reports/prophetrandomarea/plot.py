...
DATA_FILES = [
    "prophetrandomarea_scenario100_MessageStatsReport.txt",
    "prophetrandomarea_scenario300_MessageStatsReport.txt",
    "prophetrandomarea_scenario500_MessageStatsReport.txt",
    "randomprophetrandomarea_scenario100_MessageStatsReport.txt",
    "randomprophetrandomarea_scenario300_MessageStatsReport.txt",
    "randomprophetrandomarea_scenario500_MessageStatsReport.txt",
]
...
def parse_message_stats_file(filepath):
    ...
    try:
        ...
        if "randomprophet" in filename.lower():
            prophet_type = "Random Prophet"
        elif "prophet" in filename.lower():
            prophet_type = "Normal Prophet"
        else:
            print(f"Warning: Could not determine prophet type for {filename}")
            return None

        match_scenario = re.search(r"scenario(\d+)_", filename)
        if not match_scenario:
            print(f"Warning: Could not extract scenario number from {filename}")
            return None
